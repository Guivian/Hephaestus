    ...
    <div class="tickets-page">
        <div class="d-flex flex-wrap align-items-center justify-content-between mb-4">
            <div class="d-flex align-items-center">
                ...
                <div>
                    <h3 class="mb-1">Histórico de Tickets</h3>
                    <p class="text-muted mb-0">Consulte, filtre e ordene todos os pedidos registados</p>
                </div>
            </div>
        ...
        <div class="card filter-card mb-4">
            <div class="card-body">
                <div class="d-flex justify-content-between align-items-center mb-3">
                    <h4 class="card-title mb-0"><i class="mdi mdi-filter-variant mr-2 text-primary"></i>Filtros cumulativos</h4><button type="button" id="clearFilters" class="btn btn-link text-muted btn-sm">Limpar filtros</button>
                </div>
                <div class="row">
                    <div class="col-lg-3 col-md-6">
                        <div class="form-group"><label for="referenceFilter">Código / título</label>
                            <div class="input-group">
                                <div class="input-group-prepend"><span class="input-group-text"><i class="mdi mdi-magnify"></i></span></div><input id="referenceFilter" class="form-control" type="search" placeholder="SUP001 ou assunto" />
                            </div>
                        </div>
                    </div>
                    <div class="col-lg-2 col-md-3">
                        <div class="form-group"><label for="typeFilter">Tipo</label><select id="typeFilter" class="form-control">
                    ...
                    <div class="col-lg-2 col-md-3">
                        <div class="form-group"><label for="statusFilter">Estado</label><select id="statusFilter" class="form-control">
                                <option value="">Todos</option>
                                <option>Open</option>
                    ...
                    <div class="col-lg-3 col-md-6">
                        <div class="form-group"><label for="technicianFilter">Técnico</label><select id="technicianFilter" class="form-control">
                                <option value="">Todos</option>
                                <option>Por atribuir</option>
                            </select></div>
                    </div>
                    <div class="col-lg-2 col-md-3">
                        <div class="form-group"><label for="priorityFilter">Prioridade</label><select id="priorityFilter" class="form-control">
                    ...
                    <div class="col-lg-3 col-md-4">
                        <div class="form-group mb-lg-0"><label for="dateFromFilter">Abertura desde</label><input id="dateFromFilter" class="form-control" type="date" /></div>
                    </div>
                    <div class="col-lg-3 col-md-4">
                        <div class="form-group mb-lg-0"><label for="dateToFilter">Abertura até</label><input id="dateToFilter" class="form-control" type="date" /></div>
                    </div>
                    <div class="col-lg-3 col-md-4">
                        <div class="form-group mb-lg-0"><label for="userFilter">Utilizador</label><input id="userFilter" class="form-control" type="search" placeholder="Nome do requerente" /></div>
                    </div>
        ...
        <div class="card">
            <div class="card-body p-0">
                <div class="d-flex flex-wrap justify-content-between align-items-center px-4 py-3 border-bottom">
                    <div>
                        <h4 class="card-title mb-1">Tickets</h4><span id="resultSummaryTop" class="results-summary"></span>
                    </div>
                    <div class="d-flex align-items-center"><label for="pageSize" class="text-muted small mb-0 mr-2">Por página</label><select id="pageSize" class="form-control form-control-sm page-size-select">
                            <option>15</option>
                            <option>25</option>
                            <option>50</option>
                            <option>100</option>
                ...
                <div class="table-responsive">
                    <table class="table ticket-table mb-0">
                        <thead>
                            <tr>
                                <th data-sort="code">Ticket <i class="mdi mdi-swap-vertical"></i></th>
                                <th data-sort="title">Título</th>
                                <th data-sort="type">Tipo</th>
                                <th data-sort="status">Estado</th>
                                <th data-sort="technician">Técnico</th>
                                <th data-sort="priority">Prioridade</th>
                                <th data-sort="date">Data de abertura</th>
                                <th data-sort="user">Utilizador</th>
                                <th class="no-sort"></th>
                            </tr>
                        </thead>
                ...
                <div class="d-flex flex-wrap justify-content-between align-items-center px-4 py-3 border-top"><span id="resultSummaryBottom" class="results-summary mb-2 mb-md-0"></span>
                    <nav aria-label="Paginação dos tickets">
    ...
    <script>
        (function() {
            ...
            var filtered = tickets.slice(),
                ...
                sortDirection = -1;
            var controls = {
                search: document.getElementById('referenceFilter'),
            ...
            Array.from(new Set(tickets.map(function(ticket) {
                return ticket.technician;
            }).filter(function(name) {
                return name && name !== 'Por atribuir';
            }))).sort().forEach(function(name) {
                var option = document.createElement('option');
                option.value = name;
                option.textContent = name;
                controls.technician.appendChild(option);
            });
            ...
            function currentValues() {
                ...
                Object.keys(controls).forEach(function(key) {
                    values[key] = controls[key].value.trim();
                });
                return values;
            }
            ...
            function renderFilters() {
                ...
                Object.keys(v).forEach(function(key) {
                    if (v[key]) html += '<span class="active-filter">' + labels[key] + ': ' + escapeHtml(v[key]) + '<button type="button" data-clear="' + key + '" aria-label="Remover filtro ' + labels[key] + '"><i class="mdi mdi-close"></i></button></span>';
                });
            ...
            function render() {
                filtered.sort(function(a, b) {
                    var av = a[sortKey],
                        bv = b[sortKey];
                    return av === bv ? 0 : (av > bv ? 1 : -1) * sortDirection;
                });
                ...
                var first = filtered.length ? start + 1 : 0,
                    last = Math.min(start + pageSize, filtered.length),
                    summary = 'A mostrar ' + first + '–' + last + ' de ' + filtered.length + ' tickets';
                document.getElementById('resultSummaryTop').textContent = filtered.length + ' resultado(s)';
                document.getElementById('resultSummaryBottom').textContent = summary;
            ...
            [controls.search, controls.user].forEach(function(control) {
            ...
            document.getElementById('activeFilters').addEventListener('click', function(e) {
                var button = e.target.closest('[data-clear]');
                if (button) {
                    controls[button.getAttribute('data-clear')].value = '';
                    applyFilters();
                }
            });
            ...
            document.querySelector('.ticket-table thead').addEventListener('click', function(e) {
            ...
            document.getElementById('ticketRows').addEventListener('click', function(e) {
                var target = e.target.closest('[data-ticket]');
                if (target) {
                    e.preventDefault();
                    window.alert('A abrir o ticket ' + target.getAttribute('data-ticket') + '. A ligação ao detalhe será concluída na integração com o backend.');
                }
            });
            renderFilters();
            render();
